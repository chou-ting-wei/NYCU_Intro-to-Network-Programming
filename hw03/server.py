    ...
    try:
        ...
        if not public_rooms_data:
            ...
            for room in public_rooms_data:
                status_message += f"房間ID: {room['room_id']} | 創建者: {room['creator']} | 房主： {room['host']} | 遊戲類型: {room['game_type']} | 狀態: {room['status']}\n"
...
async def handle_create_room(params, username, writer):
    ...
    if game_type in ['rock_paper_scissors', 'tictactoe', 'connectfour']:
...
async def handle_show_status(writer):
    try:
        ...
        if not public_rooms_data:
            ...
            for room in public_rooms_data:
                status_message += f"房間 ID: {room['room_id']} | 創建者: {room['creator']} | 房主： {room['host']} | 遊戲類型: {room['game_type']} | 狀態: {room['status']}\n"
